...
{
    return string_format("auto_focus_point: (%d, %d)", static_cast<int>(focus_point.x), static_cast<int>(focus_point.y));
}
...
std::string to_string(image_settings const& image)
{
    if (image.format == image_format_raw)
        ...
        return string_format("image_settings: image_format=%s, jpeg_quality=%s, jpeg_size=%s, jpeg_aspect=%s",
                             to_string(image.format), to_string(image.quality), to_string(image.size), to_string(image.aspect));
}
